<template>
  ...
  >
    <div
      class="max-w-[1200px] mx-auto w-full flex items-center justify-between"
    >
      <div>
        <img
          class="sm:w-[200px] w-[120px]"
          :src="isScrolled ? '/icons/logo.svg' : '/icons/footer_logo.svg'"
          alt="agency_ai_logo"
        />
      </div>
      <nav class="nav-desktop">
        <ul
          :class="`header_nav flex items-center lg:space-x-8 space-x-4 ${
            isScrolled ? 'text-[#AC3BDE]' : 'text-[#fff]'
          }`"
        >
          <NuxtLink
            to="#quote"
            class="border-b-2 border-transparent hover:text-[#2C093B] transition duration-300"
      ...
      <div class="flex items-center space-x-3">
        <a href="#">
          <button
            class="bg-[#2C093B] sm:text-[16px] text-[13px] rounded-full text-white px-4 sm:py-3 py-2"
          >
            Contact Us
          </button>
        </a>
    ...
    <transition name="slide-fade" mode="out-in">
      <aside class="nav-mobile" v-if="mobileNavOpen" key="mobileNav">
        ...
        <nav>
          <ul
            class="header_nav flex items-start flex-col space-y-6 text-[#fff]"
          >
            <NuxtLink
